...
where 조건식
- 테이블에서 지정된 컬럼의 값을 변경 시킨다.
- where절을 생략하게된다면 테이블의 모든 행에 대해 컬럼값이 변경이된다 (주의하자)
...
-- 도서 테이블에 대한 수정사항이 데이터베이스에 영구적으로 반영된다.
COmmit;
...
where절 생략하면 테이블 모든 행이 삭제
where절 있다면 조건식을 만족하는 행만 삭제된다

- where절의 조건식은 특정 행 하나만 삭제되도록 조건식을 작성하자
    (특정 행 하나만 삭제되도록 하기 위해 조건식은 해당 테이블에서
